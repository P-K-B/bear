...
class Monitor
{
    ...
    public function displayTable()
    {
        ...
        // $show = $_GET['show'];
        $result=$this->connection->query("SELECT * FROM attacks WHERE attacker_id=$attacker AND defender_id=$defender AND declared=$declared AND resolved=$resolved ORDER BY resolved DESC");
        if (!$result) {
            die("Error ".$this->connection->connect_errno.$this->connection->connect_error);
        }
        ...
        if ($result->num_rows==0) {
            echo "<h1>This fight has already finished :(";
        } else {
            echo $display_string;
        }
        echo "<h3> ";
